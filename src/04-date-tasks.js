...
function isLeapYear(date) {
  return new Date(`February 29, ${date.getFullYear()}`).getDate() === 29;
}
...
function timeSpanToString(startDate, endDate) {
  ...
  const h = Math.floor((date / 1000 / 3600) % 60)
    .toString()
    .padStart(2, '0');
...
function angleBetweenClockHands(date) {
  const ang = Math.abs(0.5 * (60 * date.getUTCHours() - 11 * date.getUTCMinutes()));
  const angle = (ang % 360 > 180) ? 360 - (ang % 360) : ang % 360;
